...
| 平台 | 文件名 |
|------|--------|
| macOS (Apple Silicon) | `onemini-mac-arm64.dmg` / `onemini-mac-arm64.tar.gz` |
| Linux x64 | `onemini-linux-x64.tar.gz` |
| Windows x64 | `onemini-win-x64.zip` |

每个产物附带 `.sig` 签名文件；SHA256 见 `versions.json`（不再单独上传 `.sha256` 与 `SHA256SUMS`）。
